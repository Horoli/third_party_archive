...
class PageScarabPriceTableState extends State<PageScarabPriceTable> {
                ...
                child: Row(
                  children: [
                    ...List.generate(scarabConditionList.length, (index) {
                      return buildCopyButton(scarabConditionList[index], isKr)
                          .expand();
                    }),
                  ],
                ),
              ...
              if (isKr) {
                if (scarabI18n != null &&
                  ...
                  if (scarabSplit.length >= 2 &&
                      scarabSplit[0].isNotEmpty &&
                      scarabSplit[1].isNotEmpty) {
                    String part1 = scarabSplit[0].length >= 2
                        ? scarabSplit[0].substring(scarabSplit[0].length - 2)
                        : scarabSplit[0];
                ...
                if (scarabI18n != null &&
                    scarabI18n is Map &&
                    scarabI18n['regexEn'] != null) {
                  addText = scarabI18n['regexEn'].toString();
                } else {
                  String name = item.name.toLowerCase();
                  List<String> words = name.split(' ');
                  if (words.contains('of') &&
                      words.first.isNotEmpty &&
                      words.last.isNotEmpty) {
                    addText =
                        '${words[0][0]}.of.${words.last.substring(0, math.min(3, words.last.length))}';
              ...
              child: Row(
                ...
                children: [
                  Icon(Icons.content_copy_rounded,
                      size: 16, color: textColor.withAlpha(180)),
                  const SizedBox(width: 8),
                  Text(
                    '${chaosValue.toInt()}c↑ [${scarabItems.length}]',
                    style: TextStyle(
                        color: textColor,
  ...
  Widget buildScarabTooltip({PoeNinjaItem? item, required bool isKr}) {
    ...
    return Tooltip(
      ...
      child: Container(
        decoration:
            BoxDecoration(border: Border.all(color: Colors.grey.withAlpha(50))),
        child: Stack(
          children: [
            ...
            TextButton(
              ...
              onPressed: () async {
                ...
                await launchUrl(Uri.parse(
                    'https://$domain/trade/search/$currentLeague?q={"query":{"status":{"option":"available"},"type":"${scarabI18nString(item.name, isKr)}","stats":[{"type":"and","filters":[]}]},"sort":{"price":"asc"}}'));
              },
            )
          ],
        ),
  ...
  String formatPrice(double value) {
    if (value == 0) return '0';
    // 소수점 3자리까지 계산 후 불필요한 0과 소수점을 제거하여 유의미한 숫자만 노출
  ...
  List<PoeNinjaItem> getScarabItemsWithChaosValue(double chaosValue) {
    if (chaosValue >= chaosValueFirst) return getScarab.overFortyScarabItems;
    if (chaosValue >= chaosValueSecond) return getScarab.overTwentyScarabItems;
    if (chaosValue >= chaosValueThird) return getScarab.overTenScarabItems;
    if (chaosValue >= chaosValueFourth) return getScarab.overFourScarabItems;
    return [];
  }

  Color getBackgroundColor(double chaosValue) {
    if (chaosValue >= chaosValueFirst) return COLOR.TIER_1_BG;
    if (chaosValue >= chaosValueSecond) return COLOR.TIER_2_BG;
    if (chaosValue >= chaosValueThird) return COLOR.TIER_3_BG;
    if (chaosValue >= chaosValueFourth) return COLOR.TIER_4_BG;
    return COLOR.TIER_5_BG;
  }

  Color getTextColor(double chaosValue) {
    if (chaosValue >= chaosValueFirst) return COLOR.TIER_1_TEXT;
    if (chaosValue >= chaosValueSecond) return COLOR.TIER_2_TEXT;
    if (chaosValue >= chaosValueThird) return COLOR.TIER_3_TEXT;
    if (chaosValue >= chaosValueFourth) return COLOR.TIER_4_TEXT;
