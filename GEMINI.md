...
## 주요 업데이트 요약 (2026-03-17)
1.  **지능형 정규식(Regex) 엔진**: `i18n.dart`와 Fallback 알고리즘을 결합하여 영문/국문 최적화 검색식을 생성합니다.
2.  **통합 정보 인터페이스**: 우측 하단 FAB 영역에 언어 전환과 데이터 갱신 정보를 통합하여 접근성을 높였습니다.
3.  **날짜 처리 고도화**: 비표준 로케일 날짜 형식을 정규표현식으로 정제하여 일관된 포맷(`yyyy-MM-dd HH:mm`)을 보장합니다.
4.  **UI/UX 리팩토링**: 시세 버튼 디자인 개선 및 다국어 지원 범위를 앱 전반(검증 메시지 포함)으로 확대했습니다.
