...
1.  **Scarab Price Table 전면 개편**:
    - **UI/UX 개선**: 네버싱크(NeverSink) 필터 스타일의 티어별 색상(배경/텍스트)을 적용하여 가독성을 높이고, 클립보드 복사 버튼을 Material Card 기반의 세련된 디자인(아이콘, 리플 이펙트 포함)으로 리뉴얼했습니다. 가격 표시 시 1c 미만은 소수점으로, 정수는 깔끔하게 포맷팅하도록 개선했습니다.
    - **성능 최적화**: `Set` 자료구조를 도입한 O(N+M) 연산으로 필터링 속도를 개선하고, 300개 이상의 개별 `Obx` 리스너를 상위 수준으로 통합하여 초기 로딩 지연(Jank)을 제거했습니다. 데이터 가공 로직을 Controller로 이전하여 단일 리빌드 사이클을 구현했습니다.
